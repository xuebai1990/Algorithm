# python3
import numpy as np

EPS = 1e-6
PRECISION = 20

class Equation:
    def __init__(self, a, b):
        self.a = a
        self.b = b

class Position:
    def __init__(self, column, row):
        self.column = column
        self.row = row

def ReadEquation():
    size = int(input())
    if size == 0:
        exit(0)
    a = []
    b = []
    for row in range(size):
        line = list(map(float, input().split()))
        a.append(line[:size])
        b.append(line[size])
        aa = np.array(a, dtype=float)
        bb = np.array(b, dtype=float)
    return Equation(aa, bb)

def SelectPivotElement(a, used_rows, used_columns):
    # This algorithm selects the first free element.
    # You'll need to improve it to pass the problem.
    pivot_element = Position(0, 0)
    while used_rows[pivot_element.row]:
        pivot_element.row += 1
    while used_columns[pivot_element.column] or a[pivot_element.row][pivot_element.column] == 0:
        pivot_element.column += 1
    return pivot_element

def SwapLines(a, b, used_rows, used_columns, pivot_element):
    t = np.copy(a[pivot_element.column])
    a[pivot_element.column] = a[pivot_element.row]
    a[pivot_element.row] = t
    t = np.copy(b[pivot_element.column])
    b[pivot_element.column] = b[pivot_element.row]
    b[pivot_element.row] = t
    used_rows[pivot_element.column], used_rows[pivot_element.row] = used_rows[pivot_element.row], used_rows[pivot_element.column]
    pivot_element.row = pivot_element.column;
    used_rows[pivot_element.row] = True
    used_columns[pivot_element.column] = True

def ProcessPivotElement(a, b, pivot_element):
    # Write your code here
    scale = a[pivot_element.row][pivot_element.column]
    a[pivot_element.row] /= scale
    b[pivot_element.row] /= scale
    for i in range(len(a)):
        if i == pivot_element.row or a[i][pivot_element.column] == 0:
            continue
        else:
            scale = a[i][pivot_element.column]
            a[i] = a[i] - a[pivot_element.row] * scale
            b[i] = b[i] - b[pivot_element.row] * scale

def SolveEquation(equation):
    a = equation.a
    b = equation.b
    size = len(a)

    used_columns = [False] * size
    used_rows = [False] * size
    for step in range(size):
        pivot_element = SelectPivotElement(a, used_rows, used_columns)
        SwapLines(a, b, used_rows, used_columns, pivot_element)
        ProcessPivotElement(a, b, pivot_element)

    return b

def PrintColumn(column):
    size = len(column)
    for row in range(size):
        print("%.20lf" % column[row])

if __name__ == "__main__":
    equation = ReadEquation()
    solution = SolveEquation(equation)
    PrintColumn(solution)
    exit(0)
